...
public class Test01 {
     ...
     *
     *      初始化:  clinit指的是类构造器，主要作用是在类加载过程中的初始化阶段进行执行，执行内容包括静态变量初始化和静态块的执行。
     ...
     *      扩展类加载器: 用来加载java的扩展库,java虚拟机会提供一个扩展库目录,该类加载器在此目录里面查找并加载java类
     *      系统类加载器: 负责java –classpath 或 –D java.c lass.path所指的目录下的类与jar包装入工作 ，是最常用的加载器
     *
     *-----------------------------------------------------------------------------------------------------
     *  反射:
     *      1.概述
     *      java的反射机制是指在程序的运行状态中,可以构造任意一个类的对象,可以了解任意一个对象所属的类,可以了解任意一个类的
     *      成员变量和方法,可以调用任意一个对象的属性和方法,这种动态获取程序信息以及动态调用对象的功能称为java语言的反射机制
     *          反射被视为动态语言的关键
     *
     *      2.反射的第一步:
     *          获取字节码对象
     *          //1.通过类名.class获取        Xxx.Class
     *          //2.通过该类的一个实例,获取该类的class对象 x.getClass
     *          //3.通过Class类的静态方法,强制加载到某个类,获取该类的Class对象 Class.forName()
     *          //4.通过当前类的加载器去加载 Xxx.class.getClass().loadClass
     *
     *      3.Class对象三种常用方法:
     *          1.public String getName() //获取当前Class对象所代表的名字
     *          2.public String getSimpleName() //获取当前Class对象所代表的名字
     *          3.public Object newInstance() //获取当前Class的类对象
     *
     *      4.反射获取构造方法,通过构造方法创建对象
     *      5.通过反射获取成员方法,调用成员方法
     *
     * -----------------------------------------------------------------------------------------------------
     * 动态语言和静态语言
     ...
     *  动态代理设计模式
     *      1.什么是代理
     *          代理从字面上看,代理比较好理解,无非就是待处理的意思.
     *          举个例子，你在上大学的时候，总是喜欢逃课。因此，你拜托你的同学帮你答到，
     *          而自己却窝在宿舍玩游戏... 你的这个同学恰好就充当了代理的作用，代替你去上课。
     *      2.动态代理概述:
     *          动态代理的作用:拦截对真实对象方法的直接访问,增强真实对象方法的功能
     *          动态代理和静态代理区别如下:
     *              1.静态代理我们事先需要把代理类编写出来,才能编译运行
     *              2.动态代理我们不需要事先把代理类写出来,在程序运行阶段生成的对象
     *      3.重点类和方法
     *          Proxy(java提供的用于生成动态代理对象的类)
     *          动态生成一个类的代理类对象
     *          public Object newProxyInstace{
     *              ClassLoader loader,
     *              //参数一:类加载器,给当前类的类加载器即可,
     *              Class[] classes,
     *              //参数二:给代理类和被代理类要共同实现的接口的字节码文件
     *              InvocationHandle handler
     *              //参数三:处理实现类对象
     *          }
     *
     *          接口:InvocationHandler的抽象方法
     *              //1.处理方式,拦截方式
     *          public Object invoke(Object proxy,Method method,Object[]args);
     *                                  代理对象     被拦截主方法对象    前面这个方法的参数
     *      4.动态代理的优缺点
     *          1.优点:可以给任意的接口实现类创建一个动态代理对象
     *          2.缺点:只能给接口实现类创建动态代理,不能给普通类动态代理
     *
     *
     */
